/* 
    ...
    Student Email: aasavaya@gmail.com
...
form.addEventListener('submit', (e) => {
    ...
    if (messages.length > 0) 
    {
        ...
        <h3>Error! Invalid Data Entered:</h3>
        <pre>${messages.join('\r\n')}</pre>
        `;
    }
})
...
function validateName() {
    ...
    {
        areAlphabets(inputName, '- Error! Name not entered correctly - All characters should be in alphabetical Letters');
    }
}
...
function validateEmail() {
    ...
    if (nullChecker(email, 'Email')) 
    {
        ...
        {
            messages.push("- Wrong Email entered");
        }
    }    
}
...
function validateCity() {
    ...
    {
        areAlphabets(city, '- City not valid  - All characters should be alphabetical');
    }
}

function validatePostalCode() {
    let postalCode = document.getElementById('pCode');
    let validData = /^[A-Za-z]\d[A-Za-z][ -]?\d[A-Za-z]\d$/;
    if (!(postalCode.value.match(validData))) 
    {
...
function areAlphabets(element, message) {
    let validData = /^[A-Za-z\s]+$/;
    if (!(element.value.match(validData))) 
    {
